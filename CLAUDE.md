...
- Issues: `list [--assignee @me|--author @me] | view [--with-blockers N|--with-blocking N] | create | edit [--add-label/--remove-label] | close | reopen | comment | comment-edit | comment-delete | dep list|add|remove [--blocker|--blocks]`. `@me` on `--assignee`/`--author` resolves to the configured user via one extra `Whoami` call (#299). `dep` subcommand manages issue dependencies on both Forgejo and GitHub (REST landed in API version 2026-03-10; #317 / #326).
