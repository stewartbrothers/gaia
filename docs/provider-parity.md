...
| `AddIssueDependency` | ✓ | △ | Forgejo: `POST /repos/{o}/{r}/issues/{n}/dependencies` with body `{"index": M, "owner": "o", "repo": "r"}` — the owner/repo always name the dep's repo: the host's own for a same-repo edge (Forgejo resolves the body identity rather than defaulting it, so omitting the pair 404s with `IsErrRepoNotExist`, #392), or the other repo for cross-repo (#325). GitHub: `POST /repos/{o}/{r}/issues/{n}/dependencies/blocked_by` with body `{"issue_id": M}` where M is the issue's **internal stable id**, not its user-facing number; cross-repo refs target the dep's repo for the number→id resolve, then POST to the host's blocked_by endpoint with the resolved id. The Provider contract takes a `dep provider.IssueDepRef` callers think of in {Number, optional Owner+Repo} terms; the github provider transparently resolves number → id via an extra `GET /issues/{dep}` round-trip before the write. |
| `RemoveIssueDependency` | ✓ | △ | Forgejo: `DELETE /repos/{o}/{r}/issues/{n}/dependencies` with body `{"index": M, "owner": "o", "repo": "r"}` (same identity rules as Add). GitHub: `DELETE /repos/{o}/{r}/issues/{n}/dependencies/blocked_by/{issue_id}` — id in path, no body. Same number → id resolve (cross-repo aware) as Add. 404 → `exitcode.NotFound` on both. |
| `GetBranchProtection` | ✓ | ✓ | Forgejo: `GET /repos/{o}/{r}/branch_protections/{branch}`; NotFound when unprotected. GitHub: `GET /repos/{o}/{r}/branches/{branch}/protection` (#350); the binding fields nest under `required_status_checks` (read both the legacy `contexts[]` and newer `checks[]` form) and `required_pull_request_reviews.required_approving_review_count`. 404 → `NotFound` on both. |
| `SetBranchProtection` | ✓ | ✓ | Forgejo: upsert — GET to detect, then `POST .../branch_protections` (create) or `PATCH .../branch_protections/{branch}` (replace). Trimmed fields map to `enable_status_check`+`status_check_contexts`, `block_on_outdated_branch` (= strict), `required_approvals`. GitHub: single declarative `PUT .../protection` (#350) carrying the full object — `required_status_checks={strict,contexts}` (or `null` to clear), `required_pull_request_reviews={required_approving_review_count}` (or `null`), plus `enforce_admins=false` / `restrictions=null` for the knobs gaia doesn't model yet. |
| `DeleteBranchProtection` | ✓ | ✓ | Forgejo: `DELETE .../branch_protections/{branch}`. GitHub: `DELETE .../branches/{branch}/protection` (#350). 204 on both. |
| `ListWorkflowRuns` | ✓ | ✓ | Forgejo: `GET .../actions/runs` (`{total_count, workflow_runs}`); `opts.Branch`→`ref`, `opts.Status`→`status`. GitHub: same path + envelope; `opts.Branch`→`branch`, `opts.Status`→`status` (accepts status *or* conclusion values). Both trim to `[]types.WorkflowRun` newest-first (#386). |
| `GetWorkflowRun` | ✓ | ✓ | Run-ID semantics differ: Forgejo's `<run-id>` is the user-facing `index_in_repo`, resolved to the internal DB id via an extra `?run_number=N` hop; GitHub's `run_id` *is* the UI number, no resolution. `WithJobs`: Forgejo lists `/actions/tasks` and filters client-side (the endpoint ignores `run_id`); GitHub fetches `/actions/runs/{id}/jobs` directly. Both populate `Jobs[]` with status reconciled to the unified vocabulary (#386). |
| `GetWorkflowRunLogs` | n/a | ✓ | Forgejo: **n/a on v15** — no PAT-reachable logs endpoint exists (#266); returns `NotImplemented` with the run's UI URL. GitHub: fetches each job's plain-text log (`/actions/jobs/{job_id}/logs`) rather than the run-level zip, so `[]types.WorkflowRunLogs` carries job id+name with no zip parsing. `opts.FailedOnly` restricts to jobs whose conclusion is `failure` (#386). |
| `RerunWorkflowRun` | n/a | ✓ | Forgejo: **n/a on v15** — no rerun API endpoint (#267); returns `NotImplemented`. GitHub: `POST .../actions/runs/{id}/rerun` (async; 201) (#386). |
