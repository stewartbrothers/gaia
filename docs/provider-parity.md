...
| `AddIssueDependency` | ✓ | △ | Forgejo: `POST /repos/{o}/{r}/issues/{n}/dependencies` with body `{"index": M}` (M is the issue NUMBER). GitHub: `POST /repos/{o}/{r}/issues/{n}/dependencies/blocked_by` with body `{"issue_id": M}` where M is the issue's **internal stable id**, not its user-facing number. The Provider contract takes a `dep int` callers think of as a number; the github provider transparently resolves number → id via an extra `GET /issues/{dep}` round-trip before the write. Same-repo only in v1 on both forges (cross-repo follow-up gap #325). |
| `RemoveIssueDependency` | ✓ | △ | Forgejo: `DELETE /repos/{o}/{r}/issues/{n}/dependencies` with body `{"index": M}`. GitHub: `DELETE /repos/{o}/{r}/issues/{n}/dependencies/blocked_by/{issue_id}` — id in path, no body. Same number → id resolve as Add. 404 → `exitcode.NotFound` on both. |
