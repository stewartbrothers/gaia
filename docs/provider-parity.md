...
| `GetBranchProtection` | ✓ | × | Forgejo: `GET /repos/{o}/{r}/branch_protections/{branch}`; NotFound when unprotected. GitHub: `NotImplemented` in v1 — maps cleanly to `GET /repos/{o}/{r}/branches/{branch}/protection`, deferred to the #345 GitHub-parity follow-up. |
| `SetBranchProtection` | ✓ | × | Forgejo: upsert — GET to detect, then `POST .../branch_protections` (create) or `PATCH .../branch_protections/{branch}` (replace). Trimmed fields map to `enable_status_check`+`status_check_contexts`, `block_on_outdated_branch` (= strict), `required_approvals`. GitHub: `NotImplemented` (→ `PUT .../protection`, #345 follow-up). |
| `DeleteBranchProtection` | ✓ | × | Forgejo: `DELETE .../branch_protections/{branch}`. GitHub: `NotImplemented` (#345 follow-up). |
