...
| Secrets (metadata) | `gaia secrets list [--org]` — list the repo's (or org's) Actions secret **names + timestamps**, never values (the forge APIs are write-only). Confirms which secrets are configured (e.g. a release deploy key) without exposing material. Works on both Forgejo and GitHub (#371). |
| Variables | `gaia variables list [--org]` — list the repo's (or org's) Actions variables. Unlike secrets, variable **values ARE returned** (non-secret config like `TURBO_TEAM`, `TURBO_API`), so this shows `NAME  VALUE  UPDATED`. Works on both Forgejo (bare array, value in `data`) and GitHub (`{total_count, variables}`) (#375). |
| Runners (self-hosted) | `gaia runners list [--org]` — list the repo's (or org's) self-hosted Actions runners with **name, status (online/offline), busy flag, and labels**. Confirms a CI/deploy runner is live and what it can run before triggering a release. Repo-level list may be empty when runners are org/instance-scoped — use `--org` then. Works on both Forgejo and GitHub (#376). |
| Collaborators | `gaia collaborators list` — repo access audit: who has read-or-better access and at what **permission** level. GitHub returns the permission inline; Forgejo's list omits it, so gaia resolves each collaborator's permission with one extra per-user call. Works on both Forgejo and GitHub (#377). |
| Branches | `gaia branch list` and `gaia branch create <name> [--from <ref>]` — create a branch from `--from` (a branch, tag, or commit) or the repo's default branch when omitted. Works on both Forgejo and GitHub (#368). |
| Tags | `gaia tag list \| create <name> [--from <ref>] \| delete <name> [--confirm]` — manage git tags independent of releases (list a tag with no release; create/delete a bare tag). `create` tags `--from` (a branch, tag, or commit) or the default branch when omitted; `delete` is `--confirm`-gated (prints a preview otherwise). Universal git op — not capability-gated. Works on both Forgejo and GitHub (#378). |
| Branch protection | `gaia branch protection get \| set \| delete <branch>` — make a CI gate binding: `set main --required-check 'CI / Build' --strict --required-approvals 1` requires the named check (a red OR absent check blocks merge). Read the exact context strings with `gaia pr view <n> --with-ci`. Works on both Forgejo and GitHub (#345, #350). |
