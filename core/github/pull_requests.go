...
func (p *Provider) GetPullRequest(ctx context.Context, owner, repo string, n int, opts provider.GetPullRequestOptions) (*types.PullRequest, error) {
	...
	// because the check-runs lookup below is keyed off it. Serving a
	// cached PR would key the status fetch off a stale head SHA (e.g.
	// after a force-push), reporting the previous commit's CI result as
	// if it were current (#367). So bypass the cache for CI reads; plain
	// reads keep the 5-min cache.
