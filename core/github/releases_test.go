...
func TestListReleasesGHOmitsSummaryOnlyFields(t *testing.T) {
...
func TestGetReleaseGHTargetCommitishPresent(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		_ = json.NewEncoder(w).Encode(ghReleaseJSON(1, "v1.0.0", "First", false, false))
	}))
	defer srv.Close()
